	...
	\begin{minipage}{0.5\linewidth}
		...
		\end{minipage}			
            ...
            \block{Sample Attack}{
                ...
                Suppose we are targeting the loop in the firmware, shown in Fig.~\ref{loop}, outputing to a pin via \texttt{OUT}. We can count the cycles between successive outputs and use the \texttt{OUT} as our trigger, glitching the desired number of cycles after that. We know that targeting single-cycle instructions has the impact of \texttt{NOP}-ing whatever instruction follows without affecting \texttt{PC} incremention \citep{glitches_paper}, and hence we could keep glitching on \texttt{OUT}'s cycle in order to skip the \texttt{INC R17} operation and thus never make the loop condition false in order to dump as much memory as possible, i.e. view the firmware bytecode.
